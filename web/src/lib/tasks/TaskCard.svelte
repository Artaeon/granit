<script lang="ts">
  ...
  import { api, type Task , type Project, type Goal, todayISO } from '$lib/api';
  ...
  // Inline-edit popovers for tags / project / goal. Only one open at a
  // time — opening a second closes the first so the card never grows
  // multiple floating layers competing for keyboard focus.
  ...
  // the SnoozePicker pattern but lives directly on this card so the
  // popover markup can stay inline.
  $effect(() => {
...
>
  {#if !editing}
      ...
      <div class="flex-1 min-w-0">
        ...
        {#if !compact}
          <div class="flex flex-wrap items-center gap-1.5 mt-1.5 text-xs">
            ...
            {/if}
            <!-- Tags chip — click to open inline editor; even with no
                 tags set, a hover-only "+ tag" affordance appears so
                 the user never has to open the detail drawer just to
                 stick a tag on a task. Tag list lives in the popover
                 below; this chip is the trigger surface. -->
    ...
    {#if snoozePickerOpen}
      ...
      />
    {/if}
    <!-- Inline-edit popovers. Anchored absolute under the card so
         they don't escape the task list scroll container; data-inline-pop
         keeps click-outside from collapsing them on internal clicks.
         Only one renders at a time (openPopover is a single enum). -->
    {#if openPopover === 'tags'}
      <div
        data-inline-pop
