<script lang="ts">
  ...
  // Inline-edit popovers for tags / project / goal moved to
  // InlineEditPopovers.svelte. The card still needs to know whether
  // ANY popover is open so it can flip its overflow class (Fix A3),
  // hence the bindable openPopover state below.
  type PopoverKind = 'tags' | 'project' | 'goal';
  let openPopover = $state<PopoverKind | null>(null);
  ...
  // Swipe-to-action + long-press handler. Tracks horizontal drag
  // distance and surfaces a colored backing layer behind the card
  // showing what will fire if the user releases now.
  //   • Swipe right → toggle done (green ✓)
  //   • Swipe left  → snooze 1d (amber 💤)
  //   • Long-press  → open context menu (touch devices)
  // Logic lives in useSwipeGesture.svelte.ts — see imports.
  const swipe = useSwipeGesture({
    threshold: 80,
    onSwipeRight: () => void toggle(new Event('swipe-done')),
    onSwipeLeft: () => {
    ...
    // Always wire the long-press hook; check onContextMenu at fire-
    // time so a late-arriving prop (or undefined) is handled the
    // same way the original inline check did.
    onLongPress: (x, y) => onContextMenu?.(task, x, y)
  });
...
<div
  class="task-card-wrap relative rounded {openPopover && !swipe.active ? 'overflow-visible' : 'overflow-hidden'}"
...
>
  ...
  {#if swipe.active && swipe.offset !== 0}
    <div
      class="absolute inset-0 flex items-center px-3 text-sm font-semibold pointer-events-none
        {swipe.offset > 0 ? 'justify-start bg-surface0 text-success' : 'justify-end bg-surface0 text-warning'}"
      aria-hidden="true"
    >
      {#if swipe.offset > 0}
        <span class="flex items-center gap-1.5">
          <span class="text-lg">✓</span>
          {Math.abs(swipe.offset) >= swipe.threshold ? (task.done ? 'Reopen' : 'Done') : 'Swipe right…'}
        </span>
      {:else}
        <span class="flex items-center gap-1.5">
          {Math.abs(swipe.offset) >= swipe.threshold ? 'Snooze 1d' : 'Swipe left…'}
          <span class="text-lg">💤</span>
        </span>
      {/if}
    </div>
...
<div
  ...
  class:task-card--swiping={swipe.active}
      ...
      <div class="flex-1 min-w-0">
        ...
        {#if !compact}
          <div class="flex flex-wrap items-center gap-1.5 mt-1.5 text-xs">
            ...
            {/if}
            <!-- Tag / project / goal chips + their click-to-edit popovers
                 live in InlineEditPopovers.svelte. The chips render
                 inline here, in the same flex-wrap slot they always
                 occupied; the popover panels render absolutely-
                 positioned children of .task-card (the relative
                 positioning context above). -->
            <InlineEditPopovers bind:task bind:openPopover {onChanged} />
            {#if task.deadlineId}
              <span
