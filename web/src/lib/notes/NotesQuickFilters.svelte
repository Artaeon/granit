  ...
  <div class="flex items-center gap-2 px-3 py-1.5 border-b border-surface1 flex-shrink-0 bg-mantle/60 overflow-x-auto">
    {#if view === 'all'}
      {#if folderFilter}
        <button
          ...
          class="inline-flex items-center gap-1 px-2 py-0.5 rounded bg-surface0 border border-surface1 text-warning hover:bg-surface1 text-[11px] flex-shrink-0"
      ...
      {#if tagFilter}
        <button
          type="button"
          onclick={onClearTag}
          class="inline-flex items-center gap-1 px-2 py-0.5 rounded bg-surface0 border border-surface1 text-secondary hover:bg-surface1 text-[11px] flex-shrink-0"
      ...
      <span class="ml-auto inline-flex items-center gap-0.5 text-[11px] text-dim flex-shrink-0">
        ...
        <span class="inline-flex bg-surface0 border border-surface1 rounded overflow-hidden">
          {#each SORTS as s (s.id)}
            <button
              type="button"
              onclick={() => onPickSort(s.id)}
              class="px-2 py-0.5 {sortKey === s.id ? 'bg-primary text-on-primary' : 'text-subtext hover:bg-surface1'}"
            >{s.label}</button>
          {/each}
        </span>
      </span>
    {:else if view === 'search' && searchActive}
      <button
        ...
        class="ml-auto inline-flex items-center gap-1 px-2 py-0.5 rounded bg-surface0 border border-surface1 text-subtext hover:bg-surface1 text-[11px] flex-shrink-0"
