  ...
  <div class="w-full max-w-md h-fit bg-ctp-mantle rounded-xl border border-ctp-surface0 shadow-overlay overflow-hidden">
    ...
    {#if outputPath}
      ...
      <div class="px-4 py-6">
        ...
        <div class="bg-ctp-surface0 rounded-lg px-3 py-2 mb-4">
          <div class="text-[10px] text-ctp-overlay0 mb-1">Output path</div>
          <div class="text-[12px] text-ctp-blue break-all">{outputPath}</div>
        </div>
        <button on:click={() => dispatch('close')}
          class="w-full text-[12px] font-medium bg-ctp-surface0 text-ctp-text px-4 py-2 rounded-lg hover:bg-ctp-surface1 transition-colors">
          Close
        </button>
      </div>
    {:else if error}
      ...
      <div class="px-4 py-6">
        ...
        <div class="text-[12px] text-ctp-red/80 bg-ctp-surface0 rounded-lg px-3 py-2 mb-4">{error}</div>
    ...
    {:else}
      ...
      <div class="px-4 py-4 space-y-4">
        <!-- Note info -->
        <div>
          <div class="text-[10px] text-ctp-overlay0 mb-1">Note</div>
          <div class="text-sm text-ctp-blue font-medium truncate">{displayTitle}</div>
          <div class="text-[10px] text-ctp-surface2 truncate">{notePath}</div>
        </div>

        <!-- Format selector -->
        <div>
          <div class="text-[10px] text-ctp-overlay0 mb-2">Format</div>
          <div class="flex gap-2">
            <button on:click={() => format = 'html'}
              class="flex-1 text-[12px] font-medium px-3 py-2 rounded-lg border transition-colors"
        ...
        <!-- Metadata -->
        <div>
          <div class="text-[10px] text-ctp-overlay0 mb-2">Metadata</div>
          <div class="space-y-2">
            <input bind:value={customTitle} placeholder="Title"
              class="w-full px-3 py-1.5 text-[12px] bg-ctp-surface0 text-ctp-text rounded-md border border-ctp-surface1 outline-none focus:border-ctp-blue transition-colors" />
            <input bind:value={customDesc} placeholder="Description (optional)"
              class="w-full px-3 py-1.5 text-[12px] bg-ctp-surface0 text-ctp-text rounded-md border border-ctp-surface1 outline-none focus:border-ctp-blue transition-colors" />
            <div class="flex gap-2">
              <input bind:value={customTags} placeholder="Tags (comma-separated)"
                class="flex-1 px-3 py-1.5 text-[12px] bg-ctp-surface0 text-ctp-text rounded-md border border-ctp-surface1 outline-none focus:border-ctp-blue transition-colors" />
              <input type="date" bind:value={customDate}
                class="px-3 py-1.5 text-[12px] bg-ctp-surface0 text-ctp-text rounded-md border border-ctp-surface1 outline-none focus:border-ctp-blue transition-colors" />
            </div>
          </div>
        </div>
        ...
        <div class="bg-ctp-surface0/50 rounded-lg px-3 py-2">
          ...
          <div class="text-[10px] text-ctp-surface2 mt-1">Output: _blog/{format === 'html' ? '*.html' : '*.md'}</div>
        </div>

        <!-- Publish button -->
        <button on:click={publish} disabled={publishing || !notePath}
          class="w-full text-[12px] font-semibold bg-ctp-green text-ctp-crust px-4 py-2.5 rounded-lg hover:opacity-90 transition-opacity disabled:opacity-40">
          {#if publishing}
            Publishing...
          {:else}
            Publish
