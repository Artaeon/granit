  ...
  <div class="w-full max-w-2xl h-[85vh] bg-ctp-mantle rounded-xl border border-ctp-surface0 shadow-2xl flex flex-col overflow-hidden">
    ...
    <div class="flex items-center justify-between px-5 py-3 border-b border-ctp-surface0">
      <div class="flex items-center gap-3">
        ...
        <span class="text-[10px] text-ctp-overlay0">{filtered.length} notes</span>
      </div>
    ...
    <div class="px-5 py-3 border-b border-ctp-surface0 space-y-2">
      <div class="flex gap-2">
        <input type="text" bind:value={searchQuery}
          ...
          class="flex-1 bg-ctp-base border border-ctp-surface0 rounded-lg px-3 py-1.5 text-sm text-ctp-text placeholder:text-ctp-overlay0 focus:outline-none focus:border-ctp-mauve/50" />
        {#if searchQuery || tagFilter}
          <!-- svelte-ignore a11y-click-events-have-key-events -->
          <button class="px-3 py-1.5 bg-ctp-surface0 rounded-lg text-[10px] text-ctp-overlay0 hover:text-ctp-text transition-colors"
      ...
      {#if allTags.length > 0}
        <div class="flex flex-wrap gap-1.5 max-h-16 overflow-y-auto">
          {#each allTags.slice(0, 20) as tag}
            <!-- svelte-ignore a11y-click-events-have-key-events -->
            <span class="text-[10px] px-2 py-0.5 rounded-full cursor-pointer transition-colors
              {tagFilter === tag
                ? 'bg-ctp-mauve/30 text-ctp-mauve border border-ctp-mauve/50'
                : 'bg-ctp-surface0 text-ctp-overlay0 border border-transparent hover:text-ctp-text'}"
    ...
    <div class="flex-1 overflow-y-auto">
      {#if loading}
        <div class="flex items-center justify-center p-12">
          <span class="text-ctp-overlay0 text-sm">Loading timeline...</span>
        </div>
        ...
        <div class="relative px-5 py-4">
          ...
          {#each groups as group}
            ...
            <div class="relative flex items-center gap-3 mb-3 mt-2 first:mt-0">
              ...
              <span class="text-[10px] text-ctp-overlay0">{group.entries.length} notes</span>
            </div>
                  ...
                  <div class="flex items-start justify-between gap-2">
                    <div class="flex-1 min-w-0">
                      ...
                      <div class="flex items-center gap-2 mt-0.5">
                        <span class="text-[10px] text-ctp-overlay0">{formatDate(entry.date)}</span>
                        <span class="text-[10px] text-ctp-surface2">|</span>
                        <span class="text-[10px] text-ctp-overlay0">{fmtWordCount(entry.wordCount)} words</span>
                      </div>
                    </div>
                    <!-- Date badge -->
                    <div class="text-[10px] text-ctp-overlay0 bg-ctp-surface0 px-1.5 py-0.5 rounded shrink-0">
                  ...
                  {#if entry.tags && entry.tags.length > 0}
                    <div class="flex flex-wrap gap-1 mt-1.5">
                      {#each entry.tags.slice(0, 4) as tag}
                        <span class="text-[9px] text-{tagColor(tag)} bg-ctp-surface0 px-1.5 py-0.5 rounded-full">#{tag}</span>
                      {/each}
                      {#if entry.tags.length > 4}
                        <span class="text-[9px] text-ctp-overlay0">+{entry.tags.length - 4}</span>
                      {/if}
                    </div>
                  {/if}
                </div>
    ...
    <!-- Footer -->
    <div class="px-5 py-2 border-t border-ctp-surface0 flex gap-4 text-[10px] text-ctp-overlay0">
