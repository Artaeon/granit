...
<div class="fixed inset-0 z-[60] flex items-center justify-center" style="background:rgba(17,17,27,0.6);backdrop-filter:blur(10px)"
  ...
  <div class="w-full max-w-sm bg-ctp-mantle rounded-xl border border-ctp-surface0 shadow-overlay p-5"
    ...
    <h3 class="text-sm font-semibold text-ctp-text mb-4">{title}</h3>
    <input bind:this={inputEl} bind:value type="text" {placeholder}
      ...
      class="w-full px-3 py-2.5 text-sm bg-ctp-base text-ctp-text rounded-lg border border-ctp-surface0
             focus:border-ctp-blue outline-none placeholder:text-ctp-surface2 transition-colors" />
    <div class="flex justify-end gap-2 mt-4">
      <button on:click={() => dispatch('cancel')}
        class="px-4 py-2 text-[13px] font-medium text-ctp-subtext0 bg-ctp-surface0 rounded-lg
