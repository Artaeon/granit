      ...
      {#if mode === 'tags'}
        {#each tags as tag}
          <button on:click={() => selectTag(tag.name)}
            ...
            <span class="text-xs text-ctp-overlay0">{tag.count} notes</span>
          </button>
        {/each}
        {#if tags.length === 0}
          <p class="text-center text-sm text-ctp-overlay0 py-8">No tags found</p>
        {/if}
      {:else}
        ...
        {#if notesForTag.length === 0}
          <p class="text-center text-sm text-ctp-overlay0 py-8">No notes with this tag</p>
        {/if}
      {/if}
    </div>
  </div>
