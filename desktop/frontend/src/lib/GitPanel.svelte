    ...
    <div class="flex items-center justify-between px-4 py-0 border-b border-ctp-surface0">
      ...
      <div class="flex gap-1.5 items-center">
        ...
        <button on:click={() => dispatch('push')}
          class="text-[13px] font-medium bg-ctp-blue/90 text-ctp-crust px-3 py-1 rounded-md hover:bg-ctp-blue transition-colors">
          Push
        </button>
        <button on:click={() => dispatch('pull')}
          class="text-[13px] font-medium bg-ctp-mauve/90 text-ctp-crust px-3 py-1 rounded-md hover:bg-ctp-mauve transition-colors">
          Pull
        </button>
        <kbd class="text-[12px] text-ctp-overlay1 bg-ctp-surface0 px-1.5 py-0.5 rounded cursor-pointer hover:bg-ctp-surface1 transition-colors ml-1"
    ...
    {#if commitMode}
      <div class="px-4 py-3 border-b border-ctp-surface0 flex gap-2 bg-ctp-surface0/30">
        ...
        <button on:click={() => { if (commitMsg) { dispatch('commit', commitMsg); commitMsg = ''; commitMode = false } }}
          class="text-[13px] font-medium bg-ctp-green text-ctp-crust px-4 py-1.5 rounded-md hover:opacity-90 transition-opacity">
          Commit
        </button>
      </div>
    {/if}
      ...
      {#if tab === 'status'}
        {#each statusLines as line}
          <div class="flex items-center gap-3 py-1 px-2 rounded hover:bg-ctp-surface0/40 transition-colors">
            <span class="text-[12px] font-semibold w-16 text-right {statusColor(line)}">{statusLabel(line)}</span>
        ...
        {#if statusLines.length === 0}
          <div class="flex flex-col items-center py-12 gap-2">
            ...
            <p class="text-ctp-overlay1 text-sm">Working tree clean</p>
          </div>
        {/if}
      {:else if tab === 'log'}
        ...
        {/each}
        {#if logLines.length === 0}<p class="text-ctp-overlay1 text-center py-8">No commits yet</p>{/if}
      {:else}
        {#if diffText}
          {#each diffText.split('\n') as line}
          ...
          {/each}
        {:else}
          <p class="text-ctp-overlay1 text-center py-8">No changes to display</p>
        {/if}
      {/if}
    </div>
  </div>
