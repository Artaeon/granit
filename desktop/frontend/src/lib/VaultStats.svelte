      ...
      <div class="grid grid-cols-2 gap-3">
        {#each [
          ...
          <div class="bg-ctp-surface0 rounded-lg p-3">
            <div class="text-[12px] text-ctp-overlay1 uppercase tracking-wider">{label}</div>
            <div class="text-lg font-semibold text-ctp-text">{fmt(value || 0)}</div>
          </div>
        {/each}
        <div class="bg-ctp-surface0 rounded-lg p-3 col-span-2">
          <div class="text-[12px] text-ctp-overlay1 uppercase tracking-wider">Avg Links / Note</div>
        ...
        <div>
          ...
          {#each stats.topLinked as entry}
            <div class="flex items-center gap-2 py-1">
              ...
              <span class="text-xs text-ctp-overlay1 w-8 text-right">{entry.value}</span>
            </div>
          {/each}
        </div>
      {/if}
        ...
        <div>
          ...
          {#each stats.largestNotes as entry}
            <div class="flex items-center gap-2 py-1">
              ...
              <span class="text-xs text-ctp-overlay1 w-12 text-right">{fmt(entry.value)}w</span>
            </div>
          {/each}
        </div>
      {/if}
        ...
        <div>
          ...
          <div class="flex flex-wrap gap-2">
            {#each stats.topTags as tag}
              <span class="text-xs bg-ctp-surface0 text-ctp-blue px-2 py-1 rounded-full">#{tag.name} <span class="text-ctp-overlay1">{tag.value}</span></span>
            {/each}
          </div>
        </div>
      {/if}
