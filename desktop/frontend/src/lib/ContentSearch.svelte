...
<div class="fixed inset-0 z-50 flex justify-center pt-[8%]" style="background:rgba(17,17,27,0.55);backdrop-filter:blur(8px)" on:click|self={() => dispatch('close')}>
  <div class="w-full max-w-2xl h-[70vh] bg-ctp-mantle rounded-xl border border-ctp-surface0 shadow-2xl flex flex-col overflow-hidden">
    ...
    <div class="flex items-center gap-2 px-4 py-3 border-b border-ctp-surface0">
      ...
      {#if searching}
        <span class="text-[12px] text-ctp-overlay1 animate-pulse">Searching...</span>
      {/if}
      ...
      {#if results.length > 0}
        {#each groupedResults as [filePath, hits], gi}
          <div class="px-3 pt-2 pb-1">
            <span class="text-[12px] font-semibold text-ctp-blue truncate">{hits[0].title || filePath}</span>
            <span class="text-[12px] text-ctp-overlay1 ml-1">{filePath}</span>
          </div>
          {#each hits as hit, hi}
            ...
            <button
              ...
              <span class="text-[12px] text-ctp-overlay1 font-mono w-8 text-right flex-shrink-0 pt-0.5">L{hit.line}</span>
              <span class="text-[13px] text-ctp-text truncate flex-1">{hit.matchLine}</span>
            </button>
          {/each}
        {/each}
      {:else if query.length >= 2}
        <p class="text-center text-sm text-ctp-overlay1 py-12">No results found</p>
      {:else}
        <p class="text-center text-sm text-ctp-overlay1 py-12">Type at least 2 characters to search</p>
      {/if}
    </div>
