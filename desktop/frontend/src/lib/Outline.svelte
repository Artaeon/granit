    ...
    <div class="flex-1 overflow-y-auto py-2">
      {#each items as item}
        <button on:click={() => dispatch('jump', item.line)}
          ...
          <span class="text-[10px] text-ctp-overlay0 w-8 text-right font-mono">L{item.line + 1}</span>
      ...
      {/each}
      {#if items.length === 0}
        <p class="text-center text-sm text-ctp-overlay0 py-8">No headings found</p>
      {/if}
    </div>
    <div class="px-4 py-2 border-t border-ctp-surface0 text-[10px] text-ctp-overlay0">Click heading to jump</div>
  </div>
</div>
