      ...
      <div class="flex items-center gap-2">
        ...
        {#if selectedPlugin}
          <button on:click={() => selectedPlugin = null}
            class="text-[11px] text-ctp-overlay1 hover:text-ctp-text transition-colors mr-1">
            Plugins
          </button>
          <span class="text-ctp-overlay0 text-[11px]">/</span>
    ...
    <div class="flex-1 overflow-y-auto">
      {#if loading}
        <div class="flex items-center justify-center py-16">
          <span class="text-ctp-overlay0 text-sm">Loading plugins...</span>
        </div>
      {:else if selectedPlugin}
        ...
        <div class="p-4 space-y-4">
          ...
          <div class="bg-ctp-base rounded-lg p-4 border border-ctp-surface0">
            <div class="flex items-start justify-between">
              <div>
                ...
                <p class="text-[11px] text-ctp-overlay0 mt-0.5">v{selectedPlugin.version} by {selectedPlugin.author}</p>
                <p class="text-[12px] text-ctp-subtext0 mt-2">{selectedPlugin.description}</p>
              </div>
              <button on:click={() => togglePlugin(selectedPlugin?.name || '')}
                class="shrink-0 px-3 py-1 text-[11px] font-medium rounded-md transition-colors {selectedPlugin.enabled ? 'bg-ctp-green/15 text-ctp-green hover:bg-ctp-green/25' : 'bg-ctp-surface0 text-ctp-overlay0 hover:bg-ctp-surface1'}">
                {selectedPlugin.enabled ? 'Enabled' : 'Disabled'}
              </button>
            </div>
            <div class="mt-3 text-[10px] text-ctp-overlay0 font-mono truncate">
              {selectedPlugin.path}
            </div>
          </div>
          ...
          <!-- Commands -->
          <div>
            <h4 class="text-[11px] font-semibold text-ctp-subtext0 uppercase tracking-wider px-1 mb-2">Commands</h4>
              ...
              <div class="space-y-1">
                {#each selectedPlugin.commands as cmd}
                  <div class="flex items-center justify-between px-3 py-2 bg-ctp-base rounded-lg border border-ctp-surface0 hover:border-ctp-surface1 transition-colors">
                    <span class="text-[12px] text-ctp-text">{cmd}</span>
                    <button on:click={() => runCommand(selectedPlugin?.name || '', cmd)}
                      disabled={runningCommand === (selectedPlugin?.name + ':' + cmd)}
                      class="text-[10px] font-medium px-2.5 py-1 rounded bg-ctp-blue/15 text-ctp-blue hover:bg-ctp-blue/25 transition-colors disabled:opacity-50">
            ...
            {:else}
              <p class="text-[11px] text-ctp-overlay0 px-1">No commands defined</p>
            {/if}
          </div>

          <!-- Hooks -->
          <div>
            <h4 class="text-[11px] font-semibold text-ctp-subtext0 uppercase tracking-wider px-1 mb-2">Hooks</h4>
            {#if selectedPlugin.hooks && selectedPlugin.hooks.length > 0}
              <div class="flex flex-wrap gap-1.5 px-1">
                {#each selectedPlugin.hooks as hook}
                  <span class="text-[10px] px-2 py-0.5 rounded-full bg-ctp-surface0 text-ctp-subtext0 font-mono">
                    {hook}
                  </span>
                {/each}
              </div>
            {:else}
              <p class="text-[11px] text-ctp-overlay0 px-1">No hooks registered</p>
            {/if}
          </div>
        </div>
      {:else}
        ...
        {#if plugins.length === 0}
          <div class="flex flex-col items-center py-16 gap-3 px-8">
            ...
            <p class="text-ctp-overlay0 text-sm text-center">No plugins installed</p>
            <div class="text-[11px] text-ctp-overlay0 text-center space-y-1 mt-2">
              <p>Add plugins to:</p>
            ...
            {#each plugins as plugin}
              <div class="flex items-center gap-3 px-4 py-2.5 hover:bg-ctp-surface0/40 transition-colors cursor-pointer group"
                ...
                <div class="flex-1 min-w-0">
                  <div class="flex items-center gap-2">
                    <span class="text-[13px] font-medium text-ctp-text">{plugin.name}</span>
                    <span class="text-[10px] text-ctp-overlay0">v{plugin.version}</span>
                  </div>
                  <div class="text-[11px] text-ctp-overlay0 truncate">{plugin.description}</div>
                </div>

                <!-- Author -->
                <span class="text-[10px] text-ctp-overlay0 shrink-0 hidden sm:block">{plugin.author}</span>

                <!-- Toggle -->
                <button on:click|stopPropagation={() => togglePlugin(plugin.name)}
                  class="shrink-0 text-[10px] font-medium px-2 py-0.5 rounded transition-colors
                    {plugin.enabled ? 'text-ctp-green bg-ctp-green/10 hover:bg-ctp-green/20' : 'text-ctp-overlay0 bg-ctp-surface0 hover:bg-ctp-surface1'}">
